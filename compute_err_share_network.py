...
def create_batch(np_uid, np_iid, IFeature, uFeature, interaction_matrix, attn=False, user_data=True, remove_item=False):
    ...
    interaction_matrix_batch=[]
    for ii in range(cu_size):

        uid = np_uid.item(ii)
        
        a=interaction_matrix[uid-1]
        interaction_matrix_batch.append(a)

    
    interaction_matrix_batch=np.array(interaction_matrix_batch)

    return inp_u_index, inp_u_vocab, inp_i_vocab, intex_mat, interaction_matrix_batch
...
def comp_recall_userbased_4attention(model, IFeature, uFeature, interaction_matrix, val_grtr_userbased, K, device,
    ...
    # Create the batch
    train_inp_u_index, train_inp_u_vocabs, train_inp_i_vocab, intex_mat, interaction_matrix_x = create_batch(t_usr_ids, train_item_ids,
                                                                                       IFeature, uFeature,interaction_matrix,
                                                                                       user_data=False,
                                                                                       remove_item=remove_item,attn=attn)
    #interaction_matrix=interaction_matrix 
    if device == 'cuda':
        train_inp_i_vocab, interaction_matrix = torch.from_numpy(train_inp_i_vocab).cuda().float(), torch.from_numpy(interaction_matrix).cuda().float()
    else:
        train_inp_i_vocab, interaction_matrix = torch.from_numpy(train_inp_i_vocab).float(), torch.from_numpy(interaction_matrix).float()
    ...
    for count, uid in enumerate(user_ids):
        ...
        # compute the intersection of the K retrieved users and the groundtruth
        intersect = list(set(midx) & set(grtr))
        #print("uid, recall_peruser, intersect: ", uid, len(intersect) / len(grtr), intersect)
        recall = recall + len(intersect) / len(grtr)
    recall = recall / len(val_grtr_userbased)
